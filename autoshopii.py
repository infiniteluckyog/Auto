...
try:
    response = requests.get(products_url, headers=headers, timeout=15, verify = False)
...
def send_request():
    ...
    while retry_count < max_retries:
        try:
            ...
            response = requests.get(cart_url, headers=headers, cookies={'cookie': cookie}, timeout=15, verify = False)
            response.raise_for_status()
            ...
            if not checkout_token_match:
                ...
                print(f"✔️ Checkout Token: {checkout_token_match}")


# 
            web_build_id = find_between(response.text, 'sha&quot;:&quot;', '&quot;}')
            if not web_build_id:
            ...
            print("✔️ Session and tokens extracted successfully.")
            print(f"Web Build ID: {web_build_id}")
            print(f"Checkout Token Match: {checkout_token_match}")
            print(f"Session Token: {x_checkout_one_session_token}")
            print(f"Queue Token: {queue_token}")
            print(f"Stable ID: {stable_id}")
            print(f"Payment Method Identifier: {payment_method_identifier}")
            ...
            response = requests.post("https://deposit.shopifycs.com/sessions", headers=token_headers, json=payload, timeout=15, verify= False)
            response.raise_for_status()
    ...
    try:
        ...
        # (Construct the full GraphQL query payload here...)
...
# Extract gateway
        ...
        print("DEBUG: seller_proposal =", json.dumps(seller_proposal, indent=2))
        ...
        # Extract tax
                tax = "0.00"
        try:
            tax = seller_proposal.get("tax", {}).get("totalTaxAmount", {}).get("value", {}).get("amount", "") or \
                  seller_proposal.get("taxes", {}).get("totalTaxAmount", {}).get("value", {}).get("amount", "") or "0.00"

            if not tax or not re.match(r'^\d+(\.\d+)?$', tax):
                # fallback to regex extraction
                tax_search = re.search(r'"totalTaxAmount"\s*:\s*{\s*"value"\s*:\s*{\s*"amount"\s*:\s*"([^"]+)"', response.text)
                tax = tax_search.group(1) if tax_search else "0.00"
        ...
        print("✔️ Proposal step succeeded")
        print("Gateway:", gateway_result)
        print("Handle:", handle)
        ...
        # Continue processing...
        print("✔️ Proposal fetched successfully")
...
# Start the request process 4788920002538055|12|2026|543
send_request()
